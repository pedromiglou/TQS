...
public class Dip {
    ...
    private static Random generator;  // SecureRandom is preferred to Random
    ...
    final static int NNUMBERS = 5;
    final static int NSTARS = 2;

    final static int MAXNUMBER = 49;
    final static int MAXSTAR = 9;
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < 5; i++) {
            int candidate = Dip.generator.nextInt(MAXNUMBER) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
            }
        }
        for (int i = 0; i < 2; i++) {
            int candidate = Dip.generator.nextInt(MAXSTAR) + 1;
